...
We can change algorithm with `cds <- cluster_cells(cds, cluster_method = "louvain")` but in this case, we might just try altering the resolution parameter with the default `leiden` algorithm to increase the number of clusters yielded 
...
* Naive CD4 T-cells get split into two groups - cells at the root state and then cells that are intermediate in pseudotime 
* Naive CD4 T cells shift into memory CD4 T-cells
